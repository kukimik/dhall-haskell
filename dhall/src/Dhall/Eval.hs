...
eval !env t0 =
    case t0 of
        ...
        TimeHour ->
            VPrim $ \case
                VTimeLiteral d p -> VNaturalLit (getHour d)
                t                -> VTimeHour t
        TimeMinute ->
            VPrim $ \case
                VTimeLiteral d p -> VNaturalLit (getMinute d)
                t                -> VTimeMinute t
        TimeSecond ->
            VPrim $ \case
                VTimeLiteral d p -> VNaturalLit (getSecond d)
                t                -> VTimeSecond t
        TimeZone ->
            VTimeZone
        TimeZoneLiteral z ->
